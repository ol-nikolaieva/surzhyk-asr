...
| Запуск | Модель | lr | WER | CER |
|---|---|---|---|---|
| 1 | whisper-tiny | 1e-5 | 0.638 | 0.197 |
| 2 | whisper-tiny | 5e-6 | 0.684 | 0.211 |
| 3 | **whisper-small** | 1e-5 | **0.383** | **0.117** |

**Висновок.** Чим більша модель, тим якісніший результат: перехід від tiny до small знизив WER на ~0.25. Зменшення learning rate на tiny результат не покращило (за 3 епохи на малому датасеті модель недонавчилась). Кожен запуск логує `dataset_version`, тож видно, на якій версії даних тренувалась кожна модель.
